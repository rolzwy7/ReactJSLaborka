...
// dołączenie moduły usuwającego problem z zabezpieczeniem CORS
    ...
    User.count({ where: { user_name: user_name } }).then((count) => {
      if (count != 0) {
        ...
        User.create({ user_name: user_name, user_password: user_password })
          .then((new_user) => {
            // START
            // [!] Ta funkcjonalność nie została nam podana w oryginalnych plikach servera Node
            // Powiadom użytkowników online o tym, że nowy użytkownik został utworzony
            console.log("Created new user", new_user);
            console.log("Online users", onlineUsers);
            for (const [uid, wsocket] of Object.entries(onlineUsers)) {
              console.log(uid, wsocket);
              wsocket.send(
                JSON.stringify({
                  status: 3,
                  data: {
                    user_id: new_user.dataValues.user_id,
                    user_name: user_name,
                    online: false,
                  },
                })
  ...
  User.findAll({ where: { user_id: to } }).then((users) => {
    if (users.length >= 1) {
      ...
      Message.create(mes)
        .then((mes) => {
          if (user.user_id in onlineUsers) {
            onlineUsers[user.user_id].send(
              JSON.stringify({ status: 1, data: mes })
            );
          }
          if (mes.message_from_user_id !== mes.message_to_user_id) {
            if (mes.message_from_user_id in onlineUsers) {
              onlineUsers[mes.message_from_user_id].send(
                JSON.stringify({ status: 1, data: mes })
              );
            }
          }
